...
class Stft(torch.nn.Module, InversibleInterface):
    ...
    def inverse(self, input, ilens=None):
        """Custom ISTFT for ConVoiFilter."""

        # Convert real → complex (imag=0)
        if not torch.is_complex(input):
            input = input.to(torch.complex64)

        # 📌 FIX: espnet passes shape (..., T, F), but ISTFT wants (..., F, T)
        if input.dim() == 3:     # C, T, F
            input = input.permute(0, 2, 1)   # → C, F, T
        elif input.dim() == 4:   # B, C, T, F
        ...
        wavs = torch.istft(
            ...
            normalized=self.normalized,
            onesided=self.onesided,
            length=ilens.max().item() if ilens is not None else None,
        )

        return wavs.unsqueeze(0), ilens
